...
//	local AddVel 		= self.WeaponEnt:GetOwner():GetVelocity()
	...
	local emitter 		= ParticleEmitter(self.Position)
		if math.random(1, 2) == 1 then
			local particle = emitter:Add("effects/fuel_rod_bam"..math.random(1, 4), self.Position + 8 * self.Forward)
		...
		local particle = emitter:Add("effects/fuel_rod_bam", self.Position + 8 * self.Forward)
			if not IsValid(data:GetEntity()) or not IsValid(self.WeaponEnt:GetOwner()) then return end
			...
			particle:SetEndAlpha(0)
			particle:SetStartSize(math.Rand(30, 5))
			...
			particle:SetStartAlpha(math.Rand(245, 255))
			particle:SetEndAlpha(0)
			particle:SetStartSize(math.Rand(7, 5))
			particle:SetEndSize(math.Rand(15, 20))
			particle:SetRoll(math.Rand(-25, 25))
			particle:SetRollDelta(math.Rand(-0.05, 0.05))
			particle:SetColor(127, 255, 0)
