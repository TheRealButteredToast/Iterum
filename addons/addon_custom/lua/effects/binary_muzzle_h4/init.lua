function EFFECT:Init(data)
	...
	if emitter != nil then	
		local particle = emitter:Add( "effects/binary_muzzle", self.Position - self.Forward * 4 )
		...
		for i = 1,4 do
			local particle = emitter:Add( "effects/bin_muzz_2", self.Position - self.Forward * 4 )
				...
				particle:SetStartAlpha( math.Rand( 3, 3 ) )
		...
		if math.random( 1, 2 ) == 1 then
			...
			for j = 1,2 do
				...
				for i = -1,1,2 do 
					...
					local particle = emitter:Add( "effects/muzzleflash"..math.random( 1, 4 ), self.Position - 0 * self.Forward + 0 * j * i * self.Right)

						particle:SetVelocity( 0 * j * i * self.Right + AddVel )
			...
			for i = 1,2 do 
				...
				local particle = emitter:Add( "effects/muzzleflash"..math.random( 1, 4 ), self.Position + 8 * self.Forward )
					...
					particle:SetVelocity( 0 * self.Forward + 1.1 * AddVel )
