function EFFECT:Init(data)
	...
	if emitter != nil then	
		local particle = emitter:Add( "effects/hr_srs_muzzle", self.Position - self.Forward * 0 )
		if particle != nil then
			...
			particle:SetStartSize( math.random( 70, 70 ) )
			particle:SetEndSize( 10 )

			particle:SetRoll( math.Rand( 3, 3 ) )
			particle:SetRollDelta( math.Rand( 3, 3 ) )
			
			particle:SetColor( 255, 210, 168 )
		...
		for i = 1,4 do
			local particle = emitter:Add( "effects/hr_dmr_smoke", self.Position )
				...
				particle:SetStartSize( math.Rand( 130, 130 ) )
