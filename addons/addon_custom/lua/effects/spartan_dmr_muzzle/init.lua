function EFFECT:Init(data)
	...
	if emitter != nil then	
		local particle = emitter:Add( "muzzles/spartan_dmr", self.Position - self.Forward * 0 )
		if particle != nil then
			...
			particle:SetRollDelta( math.Rand( 3, 9 ) )
			
			particle:SetColor( 255, 255, 255 )
		
		for i = 1,4 do
			local particle = emitter:Add( "muzzles/spartan_dmr", self.Position )
				...
				particle:SetStartAlpha( math.Rand( 25, 20 ) )
				particle:SetEndAlpha( 0 )

				particle:SetStartSize( math.Rand( 1, 1 ) )
				particle:SetEndSize( math.Rand( 10, 10 ) )

				particle:SetRoll( math.Rand( -25, 4 ) )
				particle:SetRollDelta( math.Rand( -0.05, 9 ) )
