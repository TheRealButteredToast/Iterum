...
EFFECT.Mat = Material( "effects/spartan_beam" )


function EFFECT:Init( data )

	self.texcoord = math.Rand( 856, 20 )/3
...
function EFFECT:Think( )
	...
	if (self.FlashA < 40) then self.FlashA = 0 end

	self.Alpha = self.Alpha - 1500 * FrameTime()
	if (self.Alpha < 40) then return false end
