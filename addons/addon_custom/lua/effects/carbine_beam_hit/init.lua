...
function EFFECT:Init( data )
	...
	local emitter = ParticleEmitter( vOffset )
				...
				for i=1,5 do 
			local Flash = self.Emitter:Add( "effects/carbine_muzzle_h4-1", self.Origin )
			if (Flash) then
				...
				Flash:SetColor( 0, 255, 192)	
			end
		end


		for i=1,2 do 
			local particle = emitter:Add( "effects/carbine_muzzle_h4", vOffset )
