...
function ENT:Use(act, cal)
	if self.disabled or self:GetNWBool("locked") then return end
...
function ENT:Think()
	local crt = CurTime()
	if !self.disabled and !self:GetNWBool("locked")then
...
function ENT:setEngine(b)
	if self.disabled or self.engineDead or self:GetNWBool("locked") then b = false end
	if b then
...
function ENT:PhysicsCollide(cdat, phys)
	...
	if cdat.DeltaTime > 0.5 then
		...
		local dmg = (cdat.Speed*cdat.Speed*math.Clamp(mass, 0, 5000))/1000000000
