...
SWEP.VMPos 				= Vector(-0.25, 2, -2)
SWEP.VMAng 				= Vector(-1.5, 0, 0)
...
SWEP.Primary.Sound 			= Sound("drc.srs99am_fire")
...
SWEP.AttachmentTable = {
	AmmunitionTypes = {"drc_att_bprofile_halosniper_hvap"}
}
...
function SWEP:DoCustomInitialize()
	...
	if ply:EntIndex() == 0 then
		...
		if ply:GetModel() == "models/vuthakral/halo/custom/usp/sangheili_h3.mdl" then
			self.ViewModel = "models/vuthakral/halo/weapons/c_hum_srs99am.mdl"
		else
			self.ViewModel = "models/vuthakral/halo/weapons/c_hum_srs99am.mdl"
		end
	end
