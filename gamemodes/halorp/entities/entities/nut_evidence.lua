...
if (SERVER) then
				...
				if (inv) then
					...
					inv:add("evidence"):next(function(item)
						...
						if (IsValid(attacker)) then
							...
							item:setData("attacker", "Unknown Killer")
							item:setData("shitDeath", true)
						end

						item:setData("inflictor", inflictor and inflictor:GetClass() or "No Weapons")
						item:setData("deathTime", deathTime or "Unknown Time")
	...
	function ENT:onDrawEntityInfo(alpha)
		...
		nut.util.drawText(L"Death Evidence", x, y, ColorAlpha(nut.config.get("color"), alpha), 1, 1, nil, alpha * 0.65)
		nut.util.drawText(L"Evidence left behind from a murder.", x, y + 16, ColorAlpha(color_white, alpha), 1, 1, "nutSmallFont", alpha * 0.65)
	end
end
