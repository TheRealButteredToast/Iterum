...
ITEM.flag = nil
ITEM.errormessage = "Default Error Message."
...
ITEM.functions.Equip = {
	...
	onRun = function(item)
		...
		if(client:isArrested()) then
			client:notifyLocalized("You can't use weapons while you're arrested.")
			return false
		end
		...
		if(item.flag != nil) then
			if(!item.player:getChar():hasFlags(item.flag)) then
				client:ChatPrint(item.errormessage) 
				return false
			end
		end
