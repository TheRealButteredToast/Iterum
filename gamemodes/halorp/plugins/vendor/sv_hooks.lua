...
function PLUGIN:CanPlayerAccessVendor(client, vendor)
	...
	--client:notifyLocalized(""..vendor:GetClass() == nut_policevendor)
	if(vendor:GetClass() == "nut_vendor") then
		return true
	elseif (vendor:GetClass() == "nut_insurgentvendor") then
		return true
	elseif (vendor:GetClass() == "nut_unscvendor") then
		return true
	elseif (vendor:GetClass() == "nut_policevendor") then
		return true
	elseif (vendor:GetClass() == "nut_covvendor") then
		return true
	end
...
function PLUGIN:VendorTradeAttempt(
	...
	-- Increment our consumers charisma skill
	client:getChar():updateAttrib("cha", 0.00125)
end
