...
function ENT:getPrice(uniqueID, isSellingToVendor, client)
	...
	if(client) then
		-- Calculate any discounts from the players charisma
		local charisma = client:getChar():getAttrib("cha", 0)
		local discount
		
		if (charisma == nil) then charisma = 0 end
		
		if(charisma < 10)then
			discount = 0
		elseif(charisma < 20) then
			discount = .02
		elseif(charisma < 30) then
			discount = .04
		end
