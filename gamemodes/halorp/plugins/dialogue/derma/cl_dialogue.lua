...
function PANEL:dialogue(name, message, instant, color, topicID)
	...
	self.message:SetMouseInputEnabled( true )
	local click = false
	function self.message:DoClick()
		click = true
	end
	...
	concatMessage = ""
	timer.Create("typewriter", 0.04, string.len(parseMessage), function()
		...
		--End of message
		if (i == (table.Count(messageTable) + 1) or click == true) then
			--concatMessage = messageTable
			self.message:SetText(parseMessage)
			i = string.len(parseMessage)
			scrollToBottom()
			...
			if topicID then
				...
				if topicID == "GOODBYE" or topicID == "Join" then						-- END DECLARATIONS HERE!!!
					self:exit()
				end
