ENT.Type = "anim"
ENT.PrintName = "Flood Spread (Small)"
...
local models = {
	...
	"models/domitibingen/foliage/palms/d_palm_01/d_palm_b_01.mdl",
	"models/props_foliage/forestrock_cluster01.mdl",
	"models/domitibingen/foliage/palms/d_palm_01/d_palm_b_03.mdl",
}
...
function ENT:OnTakeDamage( dmginfo )
	...
	if(self:Health() <= 0 and !self.dead) then
		if(self.size >= 3) then
			if(self.heart) then
				local spread = ents.Create("sent_vj_flood_squad_a")
				spread:SetPos(self:GetPos() + Vector(0,0,30))
				spread:Spawn()
				spread:SetOwner(self)
			else
				local spread = ents.Create("npc_vj_flood_stalker")
				spread:SetPos(self:GetPos() + Vector(0,0,30))
				spread:Spawn()
				spread:SetOwner(self)
