...
function ENT:openInv(activator)
	...
	activator:setAction(L("Opening...", activator), OPEN_TIME, function()
		if (activator:GetPos():Distance(self:GetPos()) > 400) then
...
function ENT:Use(activator)
	...
	if !activator:getChar():hasFlags(self:GetNWString("flag")) and self:GetNWBool("raid",false) == false then activator:ChatPrint("You cannot access this!") return end
