...
function PLUGIN:PlayerSpawnedProp(client, model, entity)
	...
	if (hook.Run("CanPlayerSpawnStorage", client, entity, data) == false or (!client:IsAdmin() and !client:getChar():hasFlags('A'))) then
		return
	end
	...
	local def = storage:getStorageInfo() 
	if def then
		storage:SetNWString("flag",def.flag)
		storage:SetNWBool("raid")
	end
