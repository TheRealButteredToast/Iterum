...
function PANEL:Init()
	if (IsValid(nut.gui.refinv)) then
		nut.gui.refinv:Remove()
	end
...
vgui.Register("nutRefineInventory", PANEL, "EditablePanel")

PANEL = {}
function PANEL:Init()
	self:SetTitle("Refinery Menu")
	
	self.inv = self:Add("nutRefineInventory")
...
vgui.Register("nutRefineFrame", PANEL, "DFrame")

netstream.Hook("nutRefineFrame", function()
	if (refMenu and refMenu:IsVisible()) then
		refMenu:Close()
		refMenu = nil
	end

	refMenu = vgui.Create("nutRefineFrame")
end)