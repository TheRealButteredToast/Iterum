PLUGIN.name		= "Refinery"
PLUGIN.desc		= "A new refining system that actually works"
PLUGIN.author	= "Toast"

nut.util.include("cl_refinery.lua")

local playerMeta = FindMetaTable("Player")
local entityMeta = FindMetaTable("Entity")

function playerMeta:canRefine(refID)
...
function playerMeta:doRefine(refID)
	-- check the condition
...
function entityMeta:isREF()
	local class = self:GetClass()

	return (class == "nut_ref")
end

if CLIENT then

	netstream.Hook("refOpen", function(entity, index)
		...
		local actPanel = vgui.Create("DPanel")
			...
			function btn.DoClick()
				netstream.Start("doRefine", entity, v)
				panel:Remove()
				nut.gui.inv1:Remove()
				netstream.Start("invExit")
			end
