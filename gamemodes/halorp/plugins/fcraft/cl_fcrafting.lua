...
function PANEL:Init()
	if (IsValid(nut.gui.fcraftinv)) then
		nut.gui.fcraftinv:Remove()
	end
	
	nut.gui.fcraftinv = self
...
vgui.Register("nutFCraftInventory", PANEL, "EditablePanel")

PANEL = {}
function PANEL:Init()
	self:SetTitle("FARC Menu")
	
	self.inv = self:Add("nutFCraftInventory")
...
vgui.Register("nutFCraftFrame", PANEL, "DFrame")

netstream.Hook("nutFCraftFrame", function()
	if (farcMenu and farcMenu:IsVisible()) then
		farcMenu:Close()
		farcMenu = nil
	end

	farcMenu = vgui.Create("nutFCraftFrame")
end)