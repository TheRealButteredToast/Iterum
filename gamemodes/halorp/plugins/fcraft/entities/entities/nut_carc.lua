...
local function CanReplicateItemsForEveryone(inventory, action, context)
			...
			if (activator:getChar()) then
				activator:setAction("Opening...", 1, function()
					if (activator:GetPos():Distance(self:GetPos()) <= 200) then
						...
						netstream.Start(activator, "farcOpen", self, inventory:getID(), "V", "Covenant FARC Machine")
					end
				end)
			end
	...
	function ENT:onDrawEntityInfo(alpha)
		...
		nut.util.drawText(L"Covenant FARC Machine", x, y, ColorAlpha(nut.config.get("color"), alpha), 1, 1, nil, alpha * 0.65)
