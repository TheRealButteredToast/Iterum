...
function PANEL:Init()
	if (IsValid(nut.gui.farminv)) then
		nut.gui.farminv:Remove()
	end
	
	nut.gui.farminv = self
...
vgui.Register("nutFarmInventory", PANEL, "EditablePanel")

PANEL = {}
function PANEL:Init()
	self:SetTitle("Planter Menu")
	
	self.inv = self:Add("nutFarmInventory")
...
vgui.Register("nutFarmFrame", PANEL, "DFrame")

netstream.Hook("nutFarmFrame", function()
	if (farmMenu and farmMenu:IsVisible()) then
		farmMenu:Close()
		farmMenu = nil
	end

	farmMenu = vgui.Create("nutFarmFrame")
end)