...
local function CanReplicateItemsForEveryone(inventory, action, context)
	...
	function ENT:Initialize()
		self:SetModel("models/iterum/fruitpot.mdl")
	...
	function ENT:activate(client)
		if (client:canRefine()) then
			...
			local itemsToRemove = {}
			local item = inv:getItemCount("fruitpod")
				
			if (item == 0) then
				client:notifyLocalized("No fruit pods detected.")
				return false
			else
				table.insert(itemsToRemove, {"fruitpod", item})
			end
			
			for _, q in ipairs(itemsToRemove) do
				local items = inv:getItemsOfType("fruitpod")
				for i=1, q[2] do
	...
	function ENT:onDrawEntityInfo(alpha)
		...
		nut.util.drawText(L"Traxus Farming System Type 2f", x, y, ColorAlpha(nut.config.get("color"), alpha), 1, 1, nil, alpha * 0.65)
		nut.util.drawText(L"Automated farming system for fruits.", x, y + 16, ColorAlpha(color_white, alpha), 1, 1, "nutSmallFont", alpha * 0.65)
	end
end