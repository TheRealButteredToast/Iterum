...
function PANEL:Init()
	if (IsValid(nut.gui.labinv)) then
		nut.gui.labinv:Remove()
	end
...
vgui.Register("nutLaboratoryInventory", PANEL, "EditablePanel")

PANEL = {}
function PANEL:Init()
	self:SetTitle("Laboratory Menu")
	
	self.inv = self:Add("nutLaboratoryInventory")
...
vgui.Register("nutLabFrame", PANEL, "DFrame")

netstream.Hook("nutLabFrame", function()
	if (labMenu and labMenu:IsVisible()) then
		labMenu:Close()
		labMenu = nil
	end

	labMenu = vgui.Create("nutLabFrame")
end)