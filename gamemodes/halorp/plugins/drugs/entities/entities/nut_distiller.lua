...
local function CanReplicateItemsForEveryone(inventory, action, context)
	...
	function ENT:Initialize()
		self:SetModel("models/toast/ports/covbatt.mdl")
		self:PhysicsInit(SOLID_VPHYSICS)
		self:SetMoveType(MOVETYPE_VPHYSICS)
		self:SetUseType(SIMPLE_USE)
		self.loopsound = CreateSound(self, "ambient/levels/citadel/citadel_drone_loop3.wav")
		self.thump = CreateSound(self, "ambient/levels/labs/teleport_rings_loop2.wav")
		self.receivers = {}
	...
	function ENT:Use(activator)
		...
		if(timer.Exists("distTimer"..self:EntIndex())) then
			activator:notifyLocalized("Cycle is underway with ".. timer.TimeLeft("distTimer"..self:EntIndex()) .. " seconds left.")
			return false
		end
	...
	function ENT:activate(client)
		if (client:canRefine()) then
			...
			local itemsToRemove = {}
			local item = inv:getItemCount("beta")
				
			if (item == 0) then
				client:notifyLocalized("No beta materials detected.")
				return false
			else
				table.insert(itemsToRemove, {"beta", item})
			end
			
			for _, q in ipairs(itemsToRemove) do
				local items = inv:getItemsOfType("beta")
				for i=1, q[2] do
					inv:removeItem(items[i].id, false)
				end
			end
			
			client:notifyLocalized("Beggining Distillation Cycle...")
			self.loopsound:Play()
			self.thump:Play()
			timer.Create("distTimer"..self:EntIndex(), 600, 1, function()
				...
				client:notifyLocalized("Distillation cycle finished. Reseting Systems.")
				self.loopsound:Stop()
				self.thump:Stop()
			end)
			return true
	...
	function ENT:OnRemove()
		self.loopsound:Stop()
		self.thump:Stop()
		timer.Remove("distTimer"..self:EntIndex())
	...
	function ENT:onDrawEntityInfo(alpha)
		...
		nut.util.drawText(L"Distillation Station", x, y, ColorAlpha(nut.config.get("color"), alpha), 1, 1, nil, alpha * 0.65)
		nut.util.drawText(L"Distills beta materials into alien drugs.", x, y + 16, ColorAlpha(color_white, alpha), 1, 1, "nutSmallFont", alpha * 0.65)
	end
end