...
function PANEL:Init()
	if (IsValid(nut.gui.geninv)) then
		nut.gui.geninv:Remove()
	end
...
vgui.Register("nutGenInventory", PANEL, "EditablePanel")

PANEL = {}
function PANEL:Init()
	self:SetTitle("Generator Menu")
	
	self.inv = self:Add("nutGenInventory")
...
vgui.Register("nutGenFrame", PANEL, "DFrame")

netstream.Hook("nutGenFrame", function()
	if (genMenu and genMenu:IsVisible()) then
		genMenu:Close()
		genMenu = nil
	end

	genMenu = vgui.Create("nutGenFrame")
end)