...
function PANEL:Init()
	if (IsValid(nut.gui.printinv)) then
		nut.gui.printinv:Remove()
	end
	
	nut.gui.printinv = self
...
vgui.Register("nutPrintInventory", PANEL, "EditablePanel")

PANEL = {}
function PANEL:Init()
	self:SetTitle("Printer Menu")
	
	self.inv = self:Add("nutPrintInventory")
...
vgui.Register("nutPrintFrame", PANEL, "DFrame")

netstream.Hook("nutPrintFrame", function()
	if (printMenu and printMenu:IsVisible()) then
		printMenu:Close()
		printMenu = nil
	end

	printMenu = vgui.Create("nutPrintFrame")
end)