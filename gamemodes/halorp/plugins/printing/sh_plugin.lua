PLUGIN.name		= "Printer Palette"
PLUGIN.desc		= "A new money printing system that actually works"
PLUGIN.author	= "Toast"

nut.util.include("cl_printing.lua")

local playerMeta = FindMetaTable("Player")
local entityMeta = FindMetaTable("Entity")

function playerMeta:canPrint(PID)
...
function playerMeta:doPrint(PID)
	-- check the condition
...
function entityMeta:isFarm()
	...
	return (class == "nut_pal")
end

if CLIENT then

	netstream.Hook("printOpen", function(entity, index)
		...
		local actPanel = vgui.Create("DPanel")
			...
			function btn.DoClick()
				netstream.Start("doPrint", entity, v)
				panel:Remove()
				nut.gui.inv1:Remove()
				netstream.Start("invExit")
			end
