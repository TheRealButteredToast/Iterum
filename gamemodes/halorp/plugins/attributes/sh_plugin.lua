...
function nut.char:getChoice()
	return self:getVar("choice", nil)
end

function nut.char:setChoice(str)
	self:setVar("choice", str)
end

nut.char.registerVar("choice", {value = nil})
		...
		if (value ~= nil) then
			if (istable(value)) then
				...
				local points = hook.Run("GetStartAttribPoints", client, count)
					or nut.config.get("maxAttribs", 20)
...
if (SERVER) then
	...
	function PLUGIN:OnCharInfoSetup(panel)
		...
		for k, v in SortedPairsByMemberValue(nut.attribs.list, "name") do
			...
			local maximum = v.maxValue or nut.config.get("maxAttribs", 20)
			bar:setMax(maximum)
			bar:setReadOnly()
			bar:setText(
				Format(
