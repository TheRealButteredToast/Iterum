...
function PANEL:Think()
	if(self:getContext("faction") == 3 && self:getContext("faction") != nil) then
		self:onDisplay(true)
	else
...
function PANEL:onDisplay(boo)
	...
	if(boo && self.cTitle == nil) then
		self.cTitle = self:addLabel("Please Choose your +2 Bonus Modifier")
		self.cTitle:SetIsToggle(true)
		
		self.choice = self:Add("DComboBox")
		self.choice:SetFont("nutCharButtonFont")
		self.choice:Dock(TOP)
		self.choice:DockMargin(0, 4, 0, 0)
		self.choice:SetTall(40)
		self.choice:AddChoice("CHA")
		self.choice:AddChoice("CON")
		self.choice:AddChoice("DEX")
		self.choice:AddChoice("INT")
		self.choice:AddChoice("STR")
		self.choice.Paint = function(choice, w, h)
			nut.util.drawBlur(choice)
			surface.SetDrawColor(0, 0, 0, 100)
			surface.DrawRect(0, 0, w, h)
		end
		self.choice:SetTextColor(color_white)
		self.choice.OnSelect = function( _, _, value)
			selection = value
			self:setContext("choice", selection)
		end
		self:setContext("choice", "DEX")
	elseif(!boo && self.cTitle != nil) then
		self.cTitle:Remove()
		self.choice:Remove()
		self.cTitle = nil
		self:setContext("choice", nil)
