...
function PANEL:Init()
	if (IsValid(nut.gui.strinv)) then
		nut.gui.strinv:Remove()
	end
...
vgui.Register("nutStoreInventory", PANEL, "EditablePanel")

PANEL = {}
function PANEL:Init()
	self:SetTitle("Storage Menu")
	
	self.inv = self:Add("nutStoreInventory")
...
vgui.Register("nutStoreFrame", PANEL, "DFrame")

netstream.Hook("nutStoreFrame", function()
	if (strMenu and strMenu:IsVisible()) then
		strMenu:Close()
		strMenu = nil
	end

	strMenu = vgui.Create("nutStoreFrame")
end)